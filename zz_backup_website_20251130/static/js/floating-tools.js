...
class FloatingToolsMenu {
    ...
    init() {
        ...
        window.addEventListener('resize', () => {
            ...
            // If switching between mobile/desktop, close any open menus and update visibility
            if (wasMobile !== this.isMobile) {
                if (this.menuOpen) {
                    this.closeMenu();
                }
                // Update menu visibility based on new screen size
                this.updateMenuVisibility();
            }
        });

        // Update menu visibility on initial load
        this.updateMenuVisibility();
    ...
    updateMenuVisibility() {
        const desktopMenu = document.querySelector('.floating-tools-menu');
        const fabContainer = document.querySelector('.floating-fab-container');

        if (this.isMobile) {
            // Mobile: hide desktop menu, show FAB
            if (desktopMenu) {
                desktopMenu.style.display = 'none';
                desktopMenu.style.visibility = 'hidden';
                desktopMenu.style.pointerEvents = 'none';
            }
            if (fabContainer) {
            ...
            // Desktop: show menu, hide FAB
            if (desktopMenu) {
                desktopMenu.style.display = 'flex';
                desktopMenu.style.visibility = 'visible';
                desktopMenu.style.pointerEvents = 'auto';
            }
            if (fabContainer) {

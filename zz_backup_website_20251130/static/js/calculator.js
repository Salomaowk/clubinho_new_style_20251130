...
class Calculator {
    ...
    async initAfterDOM() {
        ...
        try {
            ...
            // Always initialize comboboxes immediately (not just when tab is active)
            // The comboboxes need to be ready even if the calculator tab isn't visible yet
            this.initializeComboboxes();

            // Always populate shipping dropdown immediately (not just when tab is active)
            this.populateShippingDropdown();

            // Always setup calculator event listeners (not just when tab is active)
            // This ensures form validation works when users fill the form
            this.setupCalculatorEventListeners();

            // Also initialize if calculator tab is already active (for other components)
    ...
    initializeComboboxes() {
        ...
        console.log('📋 Customers length:', this.customers ? this.customers.length : 0);
        console.log('📚 Assets for combobox:', this.assets);
        console.log('📚 Assets length:', this.assets ? this.assets.length : 0);

        // Always setup comboboxes, even if data is empty (will show empty dropdown)
        this.setupCombobox('customer-name', this.customers || [], 'Add New Customer');
        this.setupCombobox('book-title', this.assets || [], 'Add New Asset');

        // But also retry loading data if empty
        if (!this.customers || this.customers.length === 0) {
            console.warn('⚠️ No customers loaded, retrying loadData...');
            this.loadData().then(() => {
                console.log('✅ Data reloaded, reinitializing comboboxes');
                this.setupCombobox('customer-name', this.customers || [], 'Add New Customer');
                this.setupCombobox('book-title', this.assets || [], 'Add New Asset');
            });
        }
    ...
    setupCombobox(inputId, dataArray, addNewText) {
        ...
        // Clone the input to refresh event listeners
        ...
        // Focus event - show dropdown with all items
        newInput.addEventListener('focus', (e) => {
            console.log(`🎯 Focus event on ${inputId}, data length:`, dataArray.length);
            this.filterDropdown(newInput, dropdown, dataArray, addNewText);
        });

        // Blur event - hide dropdown after delay to allow click handling
        ...
        // Global click handler - hide dropdown when clicking outside
    ...
    filterDropdown(input, dropdown, dataArray, addNewText) {
        ...
        // If empty value, show all items; otherwise filter
        const filtered = value === ''
            ? dataArray
            : dataArray.filter(item => item.toLowerCase().includes(value));

        console.log(`📋 Filtered results: ${filtered.length} items`, filtered);

        dropdown.innerHTML = '';

        // Show up to 10 filtered items
        ...
        // Show "Add New" option only if user typed something that doesn't match existing items
        ...
        console.log(`👁️ Dropdown display: ${dropdown.style.display} (${filtered.length} items)`);
    }
    ...
    async copyToClipboard() {
        ...
        try {
            ...
            // Get current time in Tokyo timezone (JST - UTC+9)
            const now = new Date();
    ...
    async approveQuote() {
        ...
        try {
            ...
            const parts = formatter.formatToParts(now);
            const tokyoYear = parts.find(p => p.type === 'year').value;
            const tokyoMonth = parts.find(p => p.type === 'month').value;
            const tokyoDay = parts.find(p => p.type === 'day').value;
            const tokyoHours = parts.find(p => p.type === 'hour').value;
            const tokyoMinutes = parts.find(p => p.type === 'minute').value;
            const tokyoSeconds = parts.find(p => p.type === 'second').value;
            const tokyoTimestamp = `${tokyoYear}-${tokyoMonth}-${tokyoDay} ${tokyoHours}:${tokyoMinutes}:${tokyoSeconds}`;
...
function ensureCalculatorInitialized() {
    if (!calculator || !calculator.initialized) {
        ...
        // Export to global scope after creation
